...
COBBLER_AUTOYAST_URL="http://${COBBLER_SERVER}/cblr/svc/op/autoinstall/system/${TEST_MACHINE}"

mkdir -p "downloaded_autoyast_xml"
for distro in "${distros[@]}"; do
  ssh root@"${COBBLER_SERVER}" "cobbler system edit --profile=\"x86_64:${distro}-install-auto\" --name=\"${TEST_MACHINE}\""
  python3 validate-autoyastxml.py -vv --output "downloaded_autoyast_xml/autoyast_${distro}.xml" --profile "yast2_schema/yast2-schema_${distro}/rng/profile.rng" --url ${COBBLER_AUTOYAST_URL}
done
